...
class KortewegDeVriesTarget(AbstractParamODETarget):
    ...
    def kdv_solution(self, y0, t, param):
        ...
        t_span = [0,t]
        soln = solve_ivp(self.kdv_ode, t_span, y0, t_eval=np.asarray(t_span), args=(param, ), method='RK23')
        return soln
    ...
    def compute_obs_error(self, dic, compute_kdv_soln_func, compute_obs_func_model, error_func, y0_pred_list, param_pred_list, dx):
        ...
        for y0_pred, param_pred in zip(y0_pred_list, param_pred_list):
            ...
            kdv_mass_pred = dx * tf.reshape(tf.math.reduce_sum(kdv_soln_pred, axis=-1), shape=(-1,1))
            kdv_momentum_pred = dx * tf.reshape(tf.math.reduce_sum(tf.square(kdv_soln_pred), axis=-1), shape=(-1,1))
