...
class AbstractODETarget:
    ...
    def generate_init_data(self, n_traj, traj_len, seed=None):
        ...
        data_x = tf.reshape(
            tf.transpose(tf.convert_to_tensor(data_x), [1, 0, 2]),
            shape=(n_traj * traj_len, self.dim),
        )
...
class VanderPolOscillator(AbstractODETarget):
    ...
    def __init__(self, dt=1e-3, t_step=0.1, dim=2, alpha=2.0):
        super().__init__(dt, t_step, dim)
        self.alpha = alpha
        self.x_min = -5
        self.x_max = 5
    ...
    def generate_init_data(self, n_traj, traj_len, seed_x, seed_param):
        ...
        if seed_param is not None:
            ...
            param = np.random.uniform(
                size=(n_traj, self.param_dim), low=self.param_min, high=self.param_max
            )
        else:
            param = np.random.uniform(
                size=(n_traj, self.param_dim), low=self.param_min, high=self.param_max
...
class DuffingParamTarget(AbstractParamODETarget):
    def __init__(self, dt=1e-3, t_step=0.25, dim=2, param_dim=3):
    ...
    def generate_init_data(
        self, n_param, traj_len, n_traj_per_param, seed_x=123, seed_param=[1, 2, 3]
    ):
        data_x = []
        if seed_x is not None:
            np.random.seed(seed_x)
        x0 = np.random.uniform(
            size=(n_param * n_traj_per_param, self.dim), low=self.x_min, high=self.x_max
        )

        if seed_param is not None:
            np.random.seed(seed_param[0])

        delta = np.random.uniform(size=(n_param, 1), low=self.delta_min, high=self.delta_max)

        if seed_param is not None:
            np.random.seed(seed_param[1])

        alpha = np.random.uniform(size=(n_param, 1), low=self.alpha_min, high=self.alpha_max)

        if seed_param is not None:
            np.random.seed(seed_param[2])

        beta = np.random.uniform(size=(n_param, 1), low=self.beta_min, high=self.beta_max)
        param = np.concatenate((delta, alpha, beta), axis=-1)
        repeats_constant_extend_n_traj = n_traj_per_param * tf.ones(
            shape=(n_param,), dtype="int32"
        )
        ...
        data_x = tf.reshape(
            tf.transpose(tf.convert_to_tensor(data_x), [1, 0, 2]),
            shape=(n_param * traj_len * n_traj_per_param, self.dim),
        )

        repeats_constant_extend_trajlen = traj_len * tf.ones(
            shape=(n_param * n_traj_per_param,), dtype="int32"
        )
    ...
    def generate_fix_param_init_data(self, traj_len, n_traj_per_param, fixed_x0, fixed_param):
        ...
        for t in range(traj_len - 1):
            data_x.append(self.euler(data_x[t], param))
        data_x = tf.reshape(
            tf.transpose(tf.convert_to_tensor(data_x), [1, 0, 2]), shape=(traj_len, self.dim)
        )

        repeats_constant_extend_traj_len = traj_len * tf.ones(
            shape=(n_traj_per_param,), dtype="int32"
        )
...
class VanderPolMathieuTarget(AbstractParamODETarget):
    def __init__(self, mu, dt=1e-3, t_step=1e-2, dim=2, param_dim=1, k1=2, k2=2, k3=2, k4=1, w0=1):
        super().__init__(dt, t_step, dim, param_dim)
        self.x_min = -1
        self.x_max = 1
        self.u_min = -1
    ...
    def generate_init_data(self, n_traj, traj_len, seed_x, seed_param):
        ...
        # Reshape data_x and param to a flat structure
...
class FitzHughNagumoTarget(AbstractParamODETarget):
    ...
    def rhs(self, data_z, param):
        ...
        input_term = param * (
            np.exp(-((self.x + 5) ** 2)) + np.exp(-(self.x**2)) + np.exp(-((self.x - 5) ** 2))
        )
    ...
    def generate_data(self, n_traj, traj_len, seed_z, seed_param):
        ...
        ab_list = np.random.randint(low=1, high=20, size=(n_traj, 2, 1))
        ...
        if seed_param is not None:
            np.random.seed(seed_param)
        param = np.random.uniform(
            size=(traj_len, n_traj, self.param_dim), low=self.u_min, high=self.u_max
        )

        data_z.append(z0)
        for t in range(traj_len - 1):
            z_next = self.euler(data_z[-1], param[t])
            # print('z', data_z[-1])
            # print('param', param[t])
...
class ModifiedFHNTarget(FitzHughNagumoTarget):
    def __init__(
        ...
        super().__init__(
            x, dt, t_step, dim, param_dim, delta, epsilon, a0, a1, param_input
        )
    ...
    def rhs(self, data_z, data_u):
        ...
        input_term = (
            u1 * np.exp(-((self.x + 5) ** 2))
            + u2 * np.exp(-(self.x**2))
            + u3 * np.exp(-((self.x - 5) ** 2))
        )
        dvdt = vxx + v - v**3 - w + self.param_input * input_term
        dwdt = self.delta * wxx + self.epsilon * (v - self.a1 * w - self.a0)
    ...
    def basis_u_func(self, data_u):
        ...
        u1_2u2, u1_2u3, u2_2u1, u2_2u3, u3_2u1, u3_2u2 = (
            u1_2 * u2,
            u1_2 * u3,
            u2_2 * u1,
            u2_2 * u3,
...
class KortewegDeVriesTarget(AbstractParamODETarget):
    ...
    def kdv_ode(self, t, y, param):
        ...
        param_spatial = param_spatial.reshape(
            -1,
        )
    ...
    def kdv_solution(self, y0, t, param):
        ...
        t_span = [0, t]
        soln = solve_ivp(
            self.kdv_ode, t_span, y0, t_eval=np.asarray(t_span), args=(param,), method="RK23"
        )
        return soln
    ...
    def compute_obs_error(
        self,
        dic,
        compute_kdv_soln_func,
        compute_obs_func_model,
        error_func,
        y0_pred_list,
        param_pred_list,
        dx,
        ...
        for y0_pred, param_pred in zip(y0_pred_list, param_pred_list):
            ...
            kdv_mass_pred = dx * tf.reshape(
                tf.math.reduce_sum(kdv_soln_pred, axis=-1), shape=(-1, 1)
            )
            kdv_momentum_pred = dx * tf.reshape(
                tf.math.reduce_sum(tf.square(kdv_soln_pred), axis=-1), shape=(-1, 1)
            )
            ...
            model_kdv_mass_pred = compute_obs_func_model(y0_pred, param_pred, B_mass)
            model_kdv_mass_pred = model_kdv_mass_pred.reshape((-1, 1))

            model_kdv_momentum_pred = compute_obs_func_model(y0_pred, param_pred, B_momentum)
    ...
    def generate_data(self, n_traj, traj_len, seed_y0, seed_param):
        ...
        np.random.seed(seed_param)
        param_list_group = (
            np.random.uniform(low=0, high=1, size=(n_traj, traj_len, self.param_dim))
            * (self.umax - self.umin)
            + self.umin
        )
